...
require('lazy').setup({
  ...
  -- init.lua. If you want these files, they are in the repository, so you can just download them and43
...
map("n", "<leader>tt", fterm.toggle, { desc = "toggle floating terminal", silent = true })
map("t", "<ESC>", fterm.toggle, { desc = "close floating terminal", silent = true })
...
map("n", "<leader>rf", ":w !bash<CR>", { desc = "run current buffer in bash" })
...
local function my_on_attach(bufnr)
    ...
    -- custom mappings
    vim.keymap.set("n", "<C-t>", api.tree.change_root_to_node, opts("change root to node"))
end
...
map("n", "<C-<CR>>", "", { desc = "" })
map("n", "<C-m>", ":NvimTreeToggle<CR>", { desc = "toggle tree view", silent = true })
